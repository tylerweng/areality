...
var addLandmark = exports.addLandmark = function addLandmark(req, res) {
  ...
  _user2.default.findOneAndUpdate({ username: username.toLowerCase() }, { $push: { landmarkIds: landmark } }, function (err, user) {
    if (err) res.status(500).send(err);
    if (!user) res.status(401).json({ error: "User not found" });
    user.landmarkIds = [].concat(_toConsumableArray(user.landmarkIds), [landmark]);
    res.status(200).json(user);
  });
};
